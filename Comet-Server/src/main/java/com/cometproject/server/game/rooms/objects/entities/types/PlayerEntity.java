...
//        }
        ...
        if(this.playerData.isEmojiEnabled()) {
            String emoji = RoomManager.getInstance().getEmojis().getEmojiChat(message, this.getPlayer().getData().getRank());
            if(!emoji.isEmpty()) {
    ...
    private void sendEmojiChat(String emoji) {
        for (PlayerEntity playerEntity : this.getRoom().getEntities().getPlayerEntities()) {
            if (!playerEntity.getPlayer().getSettings().isUseOldChat()) {
                playerEntity.getPlayer().getSession().send(new TalkMessageComposer(this.getId(), emoji, ChatEmotion.NONE, 0));
            } else if (!playerEntity.getPlayer().getSettings().disableWhisper()) {
                playerEntity.getPlayer().getSession().send(new WhisperMessageComposer(this.getId(), emoji, 0));
            }
        }
    }
    ...
    private void sendNameChange() {
        ...
        String format = "<font color='#DC7C1B'>[%s]</font><font color='#%s'> %s</font>";
        String colour = getPlayer().getData().getNameColour();
        String tag = getPlayer().getData().getTag();
        if(!tag.isEmpty()) {
            username.append(String.format("<font color='#DC7C1B'>%s ›</font><font color='#%s'> %s</font>", new Object[]{tag, colour, getUsername()}));
        } else {
            username.append(String.format("<font color='#%s'> %s</font>", new Object[]{colour, getUsername()}));
        }
        ...
        for (PlayerEntity playerEntity : getRoom().getEntities().getPlayerEntities()) {
