...
public class Room implements Attributable, IRoom {
    ...
    private void startV75Ticking() {
        ...
        for (RoomItemFloor item : this.getItems().getByClass(JackpotTimerFloorItem.class)) {
            ...
            item.getItemData().setData(time + "");
            item.sendUpdate();
            item.saveData();
            if (item.getItemData().getData().equals("0") && lastTime != null && lastTime.isEmpty()) {
                item.getItemData().setData(lastTime);
            }

            int gameLength = Integer.parseInt(item.getItemData().getData());

            lastTime = item.getItemData().getData();
    ...
    public RoomModelFactory getLayout() { return this.layout;}

    public void setLayout(RoomModelFactory layout) {
        this.layout = layout;
    }
    ...
    public void atualizarAtual(String msc) {
        this.tocando = msc;
    }

    public void atualizarTempoJuke(int tempo) {
        this.TempoJuke = tempo;
    }

    public void atualizarDeslike(int deslk) {
        this.deslikes = deslk;
    }

    public void atualizarLike(int like) {
        this.likes = like;
    }
