    ...
    public static List<IRoomData> order(List<IRoomData> rooms, int limit) {
        try {
            rooms.sort((room1, room2) -> {
                ...
                if(RoomManager.getInstance().get(room2.getId()).getData().isVisaCount() == true || RoomManager.getInstance().get(room1.getId()).getData().isVisaCount() == true) {
                    return ((!is2Active ? 0 : RoomManager.getInstance().get(room2.getId()).getEntities().playerCount() + RoomManager.getInstance().get(room2.getId()).getEntities().getBotEntities().size()) -
                            (!is1Active ? 0 : RoomManager.getInstance().get(room1.getId()).getEntities().playerCount() + RoomManager.getInstance().get(room2.getId()).getEntities().getBotEntities().size()));
                } else {
