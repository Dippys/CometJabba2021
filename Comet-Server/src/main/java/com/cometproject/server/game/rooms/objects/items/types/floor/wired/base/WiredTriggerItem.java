...
import com.cometproject.server.game.rooms.objects.items.types.floor.wired.actions.custom.WiredCustomCollisionCase;
import com.cometproject.server.game.rooms.objects.items.types.floor.wired.actions.custom.WiredCustomForceCollision;
        ...
        try {
            ...
            for (RoomItemFloor floorItem : this.getItemsOnStack()) {
                if (floorItem instanceof WiredActionItem && wiredActions.size() <= CometSettings.wiredMaxEffects) {
                    ...
                    // this seems to bug the room cycle due to infinite loop
                    if(this instanceof WiredTriggerCollision && (floorItem instanceof WiredActionChase || floorItem instanceof WiredCustomForceCollision || floorItem instanceof WiredCustomCollisionCase))
                        continue;
            ...
            boolean hasOnStack = false;
            boolean hasOnFurni = false;
            ...
            for (Map.Entry<WiredConditionItem, AtomicBoolean> conditionState : completedConditions.entrySet()) {
                if (conditionState.getKey() instanceof WiredConditionHasFurniOn && !(conditionState.getKey() instanceof WiredNegativeConditionHasFurniOn)) {
                    if (conditionState.getValue().get()) hasSuccessfulOnStack = true;
                    hasOnStack = true;
                    continue;
                }
                if (conditionState.getKey() instanceof WiredConditionTriggererOnFurni &&
                    ...
                    if (conditionState.getValue().get()) hasSuccessfulOnFurni = true;
                    hasOnFurni = true;
                } else {
            ...
            // TODO: find a more elegant solution, maybe setting a type on the wired as OR,AND
            if(hasOnStack)
                canExecute = canExecute && hasSuccessfulOnStack;

            if(hasOnFurni)
                canExecute = canExecute && hasSuccessfulOnFurni;
