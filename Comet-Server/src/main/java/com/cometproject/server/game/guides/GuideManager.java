...
public class GuideManager implements Initialisable {
    ...
    public void startPlayerDuty(HelperSession helperSession) {
        this.sessions.put(Integer.valueOf(helperSession.getPlayerId()), helperSession);
        if (helperSession.handlesHelpRequests())
            this.activeGuides.put(Integer.valueOf(helperSession.getPlayerId()), Boolean.valueOf(false));
        if (helperSession.handlesBullyReports())
            this.activeGuardians.add(Integer.valueOf(helperSession.getPlayerId()));
    }

    public void finishPlayerDuty(HelperSession helperSession) {
        if (this.sessions.containsKey(Integer.valueOf(helperSession.getPlayerId())))
            this.sessions.remove(Integer.valueOf(helperSession.getPlayerId()));
        if (helperSession.handlesHelpRequests())
            this.activeGuides.remove(Integer.valueOf(helperSession.getPlayerId()));
        if (helperSession.handlesBullyReports())
            this.activeGuardians.remove(Integer.valueOf(helperSession.getPlayerId()));
    }
    ...
    public HelpRequest getHelpRequestByActiveGuide(final int guideId) {
        return this.activeHelpRequests.values().stream()
                .filter(p -> p.guideId == guideId)
                .findAny().orElse(null);
    }

    public HelpRequest getHelpRequestByParticipant(final int userId) {
        return this.activeHelpRequests.values().stream()
                .filter(p -> p.guideId == userId)
                .findAny().orElse(this.activeHelpRequests.get(userId));
    }

    public void closeHelpRequest(int userId) {
        this.activeHelpRequests.remove(userId);
    }
