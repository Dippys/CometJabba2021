...
public class RoomWriter {
    ...
    public static void write(IRoomData room, IComposer msg, boolean skipAuth) {
        ...
        if(RoomManager.getInstance().get(room.getId()).getData().isVisaCount() == true) {
            msg.writeInt(!isActive ? 0 : RoomManager.getInstance().get(room.getId()).getEntities().playerCount() + RoomManager.getInstance().get(room.getId()).getEntities().getBotEntities().size());
        } else {
