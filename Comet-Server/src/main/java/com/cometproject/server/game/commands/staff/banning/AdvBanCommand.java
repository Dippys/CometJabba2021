...
public class AdvBanCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        ...
        if (!BanManager.getInstance().hasBan(ipAddress, BanType.IP)) {
            BanManager.getInstance().banPlayer(BanType.IP, user.getIpAddress(), user.getPlayer().getEntity().getUsername(), 72, expire, Locale.getOrDefault("command.advban.reason", "Advertising"), client.getPlayer().getId(), client.getPlayer().getEntity().getUsername());
        }

        BanManager.getInstance().banPlayer(BanType.USER, user.getPlayer().getId() + "", user.getPlayer().getEntity().getUsername(), 72, expire, Locale.getOrDefault("command.advban.reason", "Advertising"), client.getPlayer().getId(), client.getPlayer().getEntity().getUsername());
