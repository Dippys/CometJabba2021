...
public abstract class ChatCommand {
    public static void sendNotif(String msg, Session session) {
        String notifImage = Locale.getOrDefault("image.sendnotif", "generic");
        session.send(new NotificationMessageComposer(notifImage, msg));
    }
    ...
    public static void isExecuted(Session session) {
        session.send(new NotificationMessageComposer("up", Locale.getOrDefault("command.executed", "Comando ejecutado correctamente.")));
    }
