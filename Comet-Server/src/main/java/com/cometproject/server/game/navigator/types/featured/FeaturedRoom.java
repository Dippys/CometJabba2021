...
public class FeaturedRoom {
    ...
    public void compose(IComposer msg) {
        ...
        if(RoomManager.getInstance().get(roomId).getEntities().getRoom().getData().isVisaCount() == true) {
            msg.writeInt(isActive ? RoomManager.getInstance().get(roomId).getEntities().playerCount() + RoomManager.getInstance().get(roomId).getEntities().getBotEntities().size() : 0);
        } else {
