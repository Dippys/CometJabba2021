...
public class ModToolBanUserMessageEvent implements Event {
    public void handle(Session client, MessageEvent msg) {
        ...
        if (user == null) {
            ...
            this.banPlayer(ipBan, machineBan, expire, userId, playerData.getUsername(), client.getPlayer().getId(), client.getPlayer().getEntity().getUsername(), length, message, playerData.getIpAddress(), "");
            return;
        }
        ...
        this.banPlayer(ipBan, machineBan, expire, user.getPlayer().getId(), user.getPlayer().getEntity().getUsername(), client.getPlayer().getId(), client.getPlayer().getEntity().getUsername(), length, message, user.getIpAddress(), machineBan ? user.getUniqueId() : "");
    }

    private void banPlayer(boolean ipBan, boolean machineBan, long expire, int playerId, String userBanned, int moderatorId, String userAddedBan, int length, String message, String ipAddress, String uniqueId) {
        this.updateStats(playerId);

        if (ipBan) {
            BanManager.getInstance().banPlayer(BanType.IP, ipAddress, userBanned, length, expire, message, moderatorId, userAddedBan);
        }

        if (machineBan && !uniqueId.isEmpty()) {
            BanManager.getInstance().banPlayer(BanType.MACHINE, uniqueId, userBanned, length, expire, message, moderatorId, userAddedBan);
        }

        BanManager.getInstance().banPlayer(BanType.USER, playerId + "", userBanned, length, expire, message, moderatorId, userAddedBan);
    }
