            ...
            for (ISession client : NetworkManager.getInstance().getSessions().getSessions().values()) {
                try {
                    ...
                    if (needsReward) {
                        ...
                        PlayerEntity playerEntity = (PlayerEntity) client.getPlayer().getEntity();
                                if (CometSettings.onlineRewardDuckets > 0) {
                                    client.getPlayer().getData().increaseActivityPoints(CometSettings.onlineRewardDuckets);
                                    client.sendQueue(new NotificationMessageComposer("pixel", String.format("¡Has recibido %d Píxeles!", CometSettings.onlineRewardDuckets)));
                                }
