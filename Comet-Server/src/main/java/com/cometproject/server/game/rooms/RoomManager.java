...
//        this.loadModels();
    ...
    public void unloadIdleRooms() {
        Iterator<Room> roomIterator = this.unloadingRoomInstances.values().iterator();
    ...
    private List<Integer> getActiveAvailableRooms() {
        ...
        for (Room activeRoom : this.loadedRoomInstances.values()) {
            if (!this.unloadingRoomInstances.containsKey(activeRoom.getId())) {
                if(activeRoom.getData().isVisaCount() == true) {
                    final int playerCount = activeRoom.getEntities().playerCount() + activeRoom.getEntities().getBotEntities().size();
