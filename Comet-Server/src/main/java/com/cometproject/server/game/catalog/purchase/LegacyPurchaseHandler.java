...
//                client.send(new LimitedEditionSoldOutMessageComposer());
            ...
            if ((!CometSettings.playerInfiniteBalance && (client.getPlayer().getData().getCredits() < totalCostCredits || client.getPlayer().getData().getActivityPoints() < totalCostActivityPoints)) || client.getPlayer().getData().getVipPoints() < totalCostPoints || client.getPlayer().getData().getSeasonalPoints() < totalCostSeasonal) {
            ...
            if(item.getPresetData().equals("prefix")){
                    client.getPlayer().getData().setTag(data);
                    client.getPlayer().getSession().send(new AlertMessageComposer("Has comprado tu prefijo: " + data + " exitosamente."));
                    client.getPlayer().getData().save();
                    if(data.equals("reset")) {
                        client.getPlayer().getData().setTag("");
                        client.getPlayer().getSession().send(new AlertMessageComposer("Has eliminado tu prefijo exitosamente."));
                        client.getPlayer().getData().save();
                    }
            }

            if(item.getPresetData().equals("prefix_colour")) {
                client.getPlayer().getData().setTagColour(item.getBadgeId());
            } else {
                if(item.hasBadge() && !client.getPlayer().getInventory().hasBadge(item.getBadgeId())) {
                    client.getPlayer().getInventory().addBadge(item.getBadgeId(), true);
                }
